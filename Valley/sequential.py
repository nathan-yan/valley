...
class Sequential:
    ...
    def forward(self, inputs, start = 0, end = None):
        if end == None:
            end = self.num_layers
        self.layers[start].fprop(inputs)
        for layer in range (start + 1, end):
            self.layers[layer].fprop(self.layers[layer - 1].outputs)
        return self.layers[-1].outputs
    
    def backward(self, gradients, start = None, end = 0):
        if start == None:
            start = self.num_layers
        self.layers[start - 1].bprop(gradients)
        for layer in range (start - 2, end - 1, -1):
            self.layers[layer].bprop(self.layers[layer + 1].gradients_i)
    
    def update(self, batch_size = 1, lr = 0.001):
        for layer in range (self.num_layers):
            if self.layers[layer].parameters:
                self.layers[layer].weights -= self.layers[layer].gradients_w * lr/batch_size
                self.layers[layer].bias -= self.layers[layer].gradients_b * lr/batch_size
